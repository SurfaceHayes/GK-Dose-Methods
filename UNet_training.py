...
fold_1 = date_run + '_Titan_gpu_' + gpu_num + '_UNET_DosePred_Target_KFold_Small' + '/'
...
def Conv_unit(x, nfiltx, conv_size):
    out = Conv3D(nfiltx, conv_size, padding='same')(x)
    out = BatchNorm_Activation(out)
    out = Conv3D(nfiltx, conv_size, padding='same')(out)
    out = BatchNorm_Activation(out)
    return out

def pool_unit(x, pool_size):
#     out = AveragePooling3D(pool_size, padding='same')(x)
    out = MaxPooling3D(pool_size, padding='same')(x)
    return out

def upsample_deConv_unit(x, xskip, nfiltx, conv_size, pool_size):
    out = UpSampling3D(pool_size)(x)
    out = concatenate([xskip, out], axis = 4)
    out = Conv3D(nfiltx, conv_size, padding='same')(out)
    out = BatchNorm_Activation(out)
    out = Conv3D(nfiltx, conv_size, padding='same')(out)
    out = BatchNorm_Activation(out)
...
nfilt = 32
C0 = Conv_unit(inputs, nfilt, conv_sq_size) 

nfilt = nfilt*2
P1 = pool_unit(C0, pooling_size)
C1 = Conv_unit(P1, nfilt, conv_sq_size) 

nfilt = nfilt*2
P2 = pool_unit(C1, pooling_size)
C2 = Conv_unit(P2, nfilt, conv_sq_size) 

nfilt = nfilt*2
P3 = pool_unit(C2, pooling_size)
C3 = Conv_unit(P3, nfilt, conv_sq_size) 

nfilt = nfilt*2
P4 = pool_unit(C3, pooling_size2)
C4 = Conv_unit(P4, nfilt, conv_sq_size)
C4 = Conv_unit(C4, nfilt, conv_sq_size)

nfilt = nfilt//2
U3 = upsample_deConv_unit(C4, C3, nfilt, conv_sq_size, pooling_size2)

nfilt = nfilt//2
U2 = upsample_deConv_unit(U3, C2, nfilt, conv_sq_size, pooling_size)

nfilt = nfilt//2
U1 = upsample_deConv_unit(U2, C1, nfilt, conv_sq_size, pooling_size)

nfilt = nfilt//2
U0 = upsample_deConv_unit(U1, C0, nfilt, conv_sq_size, pooling_size)
...
# model_file = fold_0 + '2022_11_10_Titan_gpu_1_HD-UNET_DosePred_Target/Pats_001-050_Target_gpu_1_ch_1_xyz_96x96x96_stepsz_20_ep_250_numpts_0_date_run_2022_11_10_mdl_UNet_snglPat_True_model.json'
# model_weights = fold_0 + '2022_11_10_Titan_gpu_1_HD-UNET_DosePred_Target/Pats_001-050_Target_gpu_1_ch_1_xyz_96x96x96_stepsz_20_ep_250_numpts_0_date_run_2022_11_10_mdl_UNet_snglPat_True_best_weights.h5'
